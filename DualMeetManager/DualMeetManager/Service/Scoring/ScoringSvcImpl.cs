...
namespace DualMeetManager.Service.Scoring
{
    ...
    public class ScoringSvcImpl : IScoringSvc
    {
        ...
        /// <summary>
        /// Tempoprary Method because field event performances are coming up as times without it. Need to implement a better solution in the future
        /// </summary>
        /// <param name="team1Abbr"></param>
        /// <param name="team2Abbr"></param>
        /// <param name="teams1and2"></param>
        /// <returns></returns>
        public static IndEvent CalculateIndFieldEventInOrder(string team1Abbr, string team2Abbr, List<Performance> teams1and2)
        {
            //
            //gather info for 1st 2nd and 3rd
            //
            ...
            if (firstPlacePerf != 0)
            {
                firstEventPoints.performance = DESI.ConvertToLengthData(firstPlacePerf);
                if (firstPlaceHeats.Count > 1)
                {
                    Console.WriteLine(numFirstPlaces + "-Way Tie for 1st");
                    firstEventPoints.athleteName = "TIE";
            ...
            if (secondPlacePerf != 0)
            {
                secondEventPoints.performance = DESI.ConvertToLengthData(secondPlacePerf);
                if (secondPlaceHeats.Count > 1)
                {
                    Console.WriteLine(numSecondPlaces + "-Way Tie for 2nd");
                    secondEventPoints.athleteName = "TIE";
            ...
            if (thirdPlacePerf != 0 && numFirstPlaces + numSecondPlaces < 3)
            {
                thirdEventPoints.performance = DESI.ConvertToLengthData(thirdPlacePerf);
                if (thirdPlaceHeats.Count > 1)
                {
                    Console.WriteLine(numThirdPlaces + "-Way Tie for 3rd");
                    thirdEventPoints.athleteName = "TIE";
        ...
        public IndEvent CalculateFieldEvent(string team1Abbr, string team2Abbr, List<Performance> perf)
        {
            ...
            return CalculateIndFieldEventInOrder(team1Abbr, team2Abbr, teams1and2);
        }
